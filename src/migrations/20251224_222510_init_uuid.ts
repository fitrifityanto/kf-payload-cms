...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "vouchers" (
  	...
  	"mulai" timestamp(3) with time zone,
  	"berakhir" timestamp(3) with time zone,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  ALTER TABLE "products" ADD CONSTRAINT "products_gambar_media_id_media_id_fk" FOREIGN KEY ("gambar_media_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "products_gambar_media_idx" ON "products" USING btree ("gambar_media_id");
  CREATE INDEX "products_updated_at_idx" ON "products" USING btree ("updated_at");
  CREATE INDEX "products_created_at_idx" ON "products" USING btree ("created_at");
  CREATE UNIQUE INDEX "vouchers_kode_idx" ON "vouchers" USING btree ("kode");
